...
- **Today is your current reading.** Today shows the course or book you're reading (the Meditations path by default), one full-screen passage per card, swiped vertically. Read as many as you like in a session, or stop any time: Today resumes at the next unread passage, marked *Resume*. A passage counts as read once you swipe past it. The first card of a session and of each new chapter, unit or week gets the illuminated initial. On Meditations days the explainer panel also holds the course's context, Greek keyword, plain-English paraphrase and the questions to carry into the day. Pick what Today reads under *Browse*: tap any author for *Read in Today* (or *Swipe through it*), or pick a course under *World traditions → Courses for Today*. Each keeps its own place. There's nothing to write.
