...
- **Save from the explainer.** Every stored explainer ends with *Save with explainer*: it saves the card (with the usual comment box) and keeps its explainer alongside. Saved shows the plain-English part under the card, the card's sheet shows the whole explainer, and *Export Markdown* includes it.
- **Text size.** Explainer text is larger by default (21px). Two sliders set the text size separately for passage cards and for explainers (85–160% each), in *You → Settings* or from the *Aa* button at the top of any explainer, each with a live sample. Saved on the device.
- **The reveal.** Each passage card, the first time it comes into view, comes to light like an illuminated page: the eyebrow's letter-spacing draws in, the gilt initial blooms out of the dark with a flash of light, the passage appears as if ink were running down the page (a soft edge descending as the blur clears), then the reference and buttons rise in. About two seconds, CSS only, once per card; Reduce Motion turns it off.
